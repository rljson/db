import { hsh } from '@rljson/hash';
import { Io } from '@rljson/io';
import { Json, JsonValue } from '@rljson/json';
import { Edit, EditProtocolRow, Rljson, timeId } from '@rljson/rljson';

import { Transform } from './transform.ts';

// @license
// Copyright (c) 2025 Rljson
//
// Use of this source code is governed by terms that can be
// found in the LICENSE file in the root of this package.
export class TransformComponent<T extends Json> implements Transform {
  constructor(
    private readonly _io: Io,
    private readonly _edit: Edit<T>,
    private readonly _tableKey: string,
  ) {}

  async run(): Promise<EditProtocolRow<any>> {
    //Value to add
    const component = this._edit.value as JsonValue & { _hash?: string };
    const rlJson = { [this._tableKey]: { _data: [component] } } as Rljson;

    //Write component to io
    await this._io.write({ data: rlJson });

    //Create EditProtocolRow
    const result = {
      //Ref to component
      [this._tableKey + 'Ref']: hsh(component as Json)._hash as string,

      //Data from edit
      route: this._edit.route,
      origin: this._edit.origin,
      previous: this._edit.previous,

      //Unique id/timestamp
      timeId: timeId(),
    } as EditProtocolRow<any>;

    return result;
  }
}
